...
<<<<<<< HEAD:Simu/AGPendule/AGPendule/optim.py
...
class Simulation(ASimulation):
    def comportement(self, x, t):
        return np.matrix([[0.0 * np.pi / 180.0]])
...
def cout(X, J0=None):
    ...
    pend = PenduleGyro()
    pend.set_state(np.matrix([th0, 0.0, np.pi / 2.0]).T, 0.0)

    kal = FiltreKalmanGyro()
    kal.set_state(
        np.matrix([th0, 0.0, 0.0]).T, np.matrix(np.diag((1.0, 1.0, 1.0))) / 10.0, 0.0
    )
...
if __name__ == "__main__":
    X0 = np.array((10.0, 5.0, 5.0))
    J0 = cout(X0)
    res = minimize(cout, X0, method="COBYLA", args=(J0,))
    print res
    cout(res.x, J0)
>>>>>>> f7dca0e4499156ae697953aa464fa16543034889:SW/MiseAuPoint/optim.py
