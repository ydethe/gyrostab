...
class PenduleGyro(ASystem):
    ...
    def __init__(self):
        ...
        self.createParameter("omega", 10000.0 * 2 * np.pi / 60)
        self.createParameter("OB", 16.6e-2)
        self.createParameter("OG", 14.56e-2)
        self.createParameter("J", 378.0)
        self.createParameter("g0", 9.81)

        self.createParameter("rho", 2700.0)
        ...
        D = (
            (6 * self.J - self.hauteur ** 2 * masse) * H2
            - 2 * self.OB ** 2 * self.hauteur ** 2 * masse
        ) / (6 * H2)
    ...
    def transition(self, t: float, x: np.array, u: np.array) -> np.array:
        ...
        dth2 = (
            12 * self.masse_tot * self.OG * self.g0 * sin(th)
            + (
                (
                    -6 * np.pi * dps1 * dth1 * self.hauteur * cos(ps)
                    - 6 * np.pi * dps1 * self.hauteur * self.omega
                )
                * sin(ps)
                * self.r2 ** 4
                + 2
            ...
            * self.rho
            ...
            (
                (3 * np.pi * self.hauteur * sin(ps) ** 2 - 6 * np.pi * self.hauteur)
                * self.r2 ** 4
                + (
                    -np.pi * self.hauteur ** 3 * sin(ps) ** 2
                    - 12 * np.pi * self.OB ** 2 * self.hauteur
                )
                * self.r2 ** 2
                + (6 * np.pi * self.hauteur - 3 * np.pi * self.hauteur * sin(ps) ** 2)
                * self.r1 ** 4
                + (
                    np.pi * self.hauteur ** 3 * sin(ps) ** 2
                    + 12 * np.pi * self.OB ** 2 * self.hauteur
