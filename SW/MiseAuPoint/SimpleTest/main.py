...
def main():
    ...
    # Init système
    sys = SystemeTest(1.0 / fs)
    sys.set_state(np.matrix([2.0, 0.0]).T, 0.0)

    # Init Kalman
    w = np.sqrt(-sys.A) / fs
    F = np.matrix([[cos(w), sin(w) / (w * fs)], [-w * fs * sin(w), cos(w)]])
    B = np.matrix([sys.K / (w * fs) ** 2 * (1.0 - cos(w)), sys.K / (w * fs) * sin(w)]).T
    H = np.matrix([[1.0, 0.0]])
    ...
    kal.set_matrices(F, B, H, Q, R, 1.0 / fs)
    kal.set_state(np.matrix([2.0, 0.0]).T, np.matrix(np.diag((1.0, 1.0))) / 10.0, 0.0)

    c = Capteur()

    ctrl = Controller(1.0 / fs, sys, c, kal)
